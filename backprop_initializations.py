...
class ActivationFunction:
    def __init__(self, func, gradient, variance_func):
        self.func = func
    ...
    def weight_variance(self, layers, layer_num):
        return self.variance_func(layers, layer_num)
...
def _lecun_variation(layers, layer_num):
    return 1 / layers[layer_num - 1]

linear = ActivationFunction(_linear, _linear_grad, _lecun_variation)
...
def _he_variance(layers, layer_num):
    return 2 / layers[layer_num - 1]

relu = ActivationFunction(_relu, _relu_grad, _he_variance)
...
leakyrelu = ActivationFunction(_leakyrelu, _leakyrelu_grad, _he_variance)
...
def _xavier_variance(layers, layer_num):
    return 2 / (layers[layer_num - 1] + layers[layer_num])

sigmoid = ActivationFunction(_sigmoid, _sigmoid_grad, _xavier_variance)
...
tanh = ActivationFunction(_tanh, _tanh_grad, _xavier_variance)
...
class Network:
    ...
    def initialize_weights(self):
        ...
        for i in range(1, len(self.layer_sizes)):
            variance = self.activations[i - 1].weight_variance(self.layer_sizes, i)
...
def main():
    network = Network([1, 16, 16, 16, 1], [leakyrelu, tanh, tanh, sigmoid])
